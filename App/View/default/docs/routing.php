<div class="continer-fluid content-box docs-page">
    ...
    <div class="row-fluid">
        ...
        <article>
            ...
            <p>Routes are an integral part of web application development. They make way for nice clean urls such as <b>/blog/view/5543</b> instead of something like <b>/blog.php?Action=view&article=5543</b>.</p>
            <p></p>
            ...
            <p class="section-title">The Details</p>
            <p>Lets talk about the structure of a route, you have a route name, pattern, defaults and requirements.</p>
            <p><b>Name:</b><br>This is a symbolic name to easily refer to this actual from different contexts in your application. Examples of route names are <b>Homepage, Blog_View, Profile_Edit</b>. These are extremely useful if you want to just redirect a user to a page like the login page, you can redirect them to <b>User_Login</b>. If you are in a template file and want to generate a link you can refer to the route name and it will be created for you. The good part about this is you can maintain the routes via your <b>routes.yml</b> file and your entire system updates.</p>
            <p><b>Pattern:</b><br>This is the URI pattern that if present will activate your route. In this example we're targeting the homepage. This is where you can specify params like <b>{id}</b> or <b>{username}</b>. You can make URLs like <b>/article/{id}</b> or <b>/profile/{username}</b></p>
            <p><b>Defaults:</b><br>This is the important part, The syntax is <b>Module:Controller:action</b>. So if you specify <b>Application:Blog:show</b> then this will execute the following class path: <b>/modules/Application/Controller/Blog->showAction()</b>. Notice how the method has a suffix of Action, this is so you can have lots of methods on your controller but only the ones ending in Action() will be executable from a route.</p>
            <p><b>Requirements: </b><br>This is where you can specify things like the request method being POST or PUT. You can also specify rules for the parameters you created above in the <b>pattern</b> section. Such as {id} being numeric, or {lang} being in a whitelist of values such as <b>en</b>|<b>de</b>|<b>it</b>.</p>
            ...
            <p class="section-title">Basic Routes</p>
            <pre><code>
Homepage:
    pattern:  /
...
Blog_Show:
    ...
    defaults: { _controller: "Application:Blog:index", pageNum: 1}
            ...
            <p class="section-title">Routes with requirements</p>
...
class Blog extends BaseController {
    ...
    public function showAction() {
        ...
        $profileUrl = $this->generateUrl('Blog_Post', array('id' => 45464);
            ...
            <p class="section-title">Redirecting to routes</p>
            <pre><code>
&lt;?php
namespace Application\Controller;
